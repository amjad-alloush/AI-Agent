...
def reason(
    ...
    if (
        "processed_data" in user_input
        and "cleaned_text" in user_input["processed_data"]
    ):
...
def _prepare_context(
    ...
    chat_history = []
    if (
        "short_term" in memory_context
        and "recent_history" in memory_context["short_term"]
    ):
        for interaction in memory_context["short_term"]["recent_history"]:
            if (
                "user_input" in interaction
                and "processed_data" in interaction["user_input"]
            ):
                user_text = interaction["user_input"]["processed_data"].get(
                    "cleaned_text", ""
                )
                chat_history.append({"role": "user", "content": user_text})
            if (
                "agent_response" in interaction
                and "response_to_user" in interaction["agent_response"]
            ):
...
def _parse_llm_response(self, llm_response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for tool_name, params_str in tool_matches:
        try:
            ...
            actions.append({"tool": tool_name, "parameters": params})
    ...
    response_to_user = re.sub(
        reasoning_pattern, "", response_to_user, flags=re.DOTALL
    ).strip()
    return {"reasoning": reasoning, "actions": actions, "response": response_to_user}